...
const gcloud = spawnSync('gcloud', [
  ...
  'list-tags',
  "--format='json'",
  'gcr.io/hartonline-cloud/home-hub-api'
])
if (gcloud.status === 0) {
  ...
  if (arr.length > MAX_CONTAINERS) {
    ...
    for (var i = MAX_CONTAINERS; i < arr.length; i++) {
      const image = arr[i]
      console.log(`Deleting image gcr.io/hartonline-cloud/home-hub-api@${image.digest}`)
      const d = spawnSync('gcloud', [
        'container',
        'images',
        'delete',
        `gcr.io/hartonline-cloud/home-hub-api@${image.digest}`,
        '--force-delete-tags'
      ])
      if (d.status === 0) {
        console.log(`Successfully deleted image gcr.io/hartonline-cloud/home-hub-api@${image.digest}`)
      } else {
        console.error(`Failed to delete image gcr.io/hartonline-cloud/home-hub-api@${image.digest}`)
